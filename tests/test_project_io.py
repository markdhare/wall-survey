...
def test_portable_project_round_trip(tmp_path):
    ...
    project = SurveyProject(name="Test", references=[Reference(name="Air", runs=[Run(source=str(source))])], locations=[Location(label="A1", x_m=.1, y_m=.2, runs=[Run(source=str(source))])])
